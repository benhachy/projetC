...
void			ei_draw_text		(ei_surface_t		surface,
		...
		uint32_t color_text = *(origine_text + (uint32_t)(x_max_text*j) + (uint32_t)(i)); // couleur de text[i][j]
...
void			ei_fill			(ei_surface_t		surface,
    ...
    if (clipper != NULL) {
        ...
        x_max_surface = taille_surface.width;
    }
    ...
    if (alpha == EI_FALSE) {
        for (int i = start_src_i; i < taille_src.width; i++) {
            for (int j = start_src_j; j < taille_src.height; j++) {
                uint32_t color_src = *(origine_src + (uint32_t) (x_max_src * j) +
                                       (uint32_t) (i)); // couleur de source[i][j]
    ...
    if (alpha == EI_TRUE) {
        for (int i = start_src_i; i < taille_src.width; i++) {
            for (int j = start_src_j; j < taille_src.height; j++) {
                uint32_t color_src = *(origine_src + (uint32_t) (x_max_src * j) +
                                       (uint32_t) (i)); // couleur de source[i][j]
                uint32_t color_dst = *(origine_dst + (uint32_t) (x_max_src * j) +
                                       (uint32_t) (i)); // couleur de destination[i][j]
